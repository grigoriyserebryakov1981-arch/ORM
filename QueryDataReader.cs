...
namespace Atdi.CoreServices.EntityOrm
{
    internal sealed class QueryDataReader : Atdi.Contracts.CoreServices.DataLayer.IDataReader
    {
        ...
        private readonly IFieldMetadata[] _fields; 
        private readonly DataTypeSystem _dataTypeSystem;

        public QueryDataReader(IEngineDataReader dataReader, IFieldMetadata[] fields, DataTypeSystem dataTypeSystem)
        {
        ...
        private T GetInternalValue<T>(int ordinal)
        {
			...
			if (dataType.CodeVarType == DataType.ClrEnum )
            {
	            var enumType = field.CodeType;
	            var enumValue = _dataTypeSystem.GetDecoder<System.Enum>(dataType).DecodeAsType(_dataReader, ordinal, enumType);
	            return (T)enumValue;
            }

            if (dataType.CodeVarType == DataType.ClrType)
            {
	            if (dataType.ContentType == StoreContentType.Json)
	            {
		            var jsonValueType = field.CodeType;
		            var jsonValue = _dataTypeSystem.GetDecoder<object>(dataType).DecodeAsType(_dataReader, ordinal, jsonValueType);
		            return (T)jsonValue;
	            }
			...
			return _dataTypeSystem.GetDecoder<T>(field.DataType).DecodeAs(_dataReader, ordinal);
        }
    }
    ...
    internal sealed class QueryDataReader<TModel> : IDataReader<TModel>
    {
        ...
        public T GetObject<T>(Expression<Func<TModel, T>> columnExpression)
        {
	        var type = typeof(T);
	        ...
	        if (type == typeof(int))
	        {
		        var resultAsTyped = GetInternalValue2<int?>(columnExpression.Body);
		        if (!resultAsTyped.HasValue)
		        {
			        return default(T);
		        }
		        return (T)(object)resultAsTyped.Value;
	        }
			else if (type == typeof(long))
	        {
		        var resultAsLong = GetInternalValue2<long?>(columnExpression.Body);
		        if (!resultAsLong.HasValue)
		        {
			        return default(T);
		        }
		        return (T)(object)resultAsLong.Value;
			}
	        var result = GetInternalValue<T, T>(columnExpression);
	        return result;
        }
        ...
        private TResult GetInternalValue<TResult, TExprResult>(Expression<Func<TModel, TExprResult>> columnExpression)
        {
            ...
            if (dataType.CodeVarType == DataType.ClrEnum)
            {
	            var enumType = columnExpression.Body.GetMemberType();
	            var enumValue = _dataTypeSystem.GetDecoder<System.Enum>(dataType).DecodeAsType(_dataReader, ordinal, enumType);
			...
			if (dataType.CodeVarType == DataType.ClrType)
            {
				if (dataType.ContentType == StoreContentType.Json)
				{
					var jsonValueType = columnExpression.Body.GetMemberType();
					var jsonValue = _dataTypeSystem.GetDecoder<object>(dataType).DecodeAsType(_dataReader, ordinal, jsonValueType);
					return (TResult)jsonValue;
				}
        ...
        private TResult GetInternalValue2<TResult>(Expression columnExpressionBody)
        {
	        if (columnExpressionBody == null)
	        {
		        throw new ArgumentNullException(nameof(columnExpressionBody));
	        }

	        var fieldPath = columnExpressionBody.GetMemberName();
	        var ordinal = _dataReader.GetOrdinalByPath(fieldPath);
	        if (_dataReader.IsDBNull(ordinal))
	        {
		        return default(TResult);
	        ...
	        if (dataType.CodeVarType == DataType.ClrEnum)
	        {
		        var enumType = columnExpressionBody.GetMemberType();
		        var enumValue = _dataTypeSystem.GetDecoder<System.Enum>(dataType).DecodeAsType(_dataReader, ordinal, enumType);
		        return (TResult)enumValue;
	        }
	        ...
	        if (dataType.CodeVarType == DataType.ClrType)
	        {
		        if (dataType.ContentType == StoreContentType.Json)
		        {
			        var jsonValueType = columnExpressionBody.GetMemberType();
			        var jsonValue = _dataTypeSystem.GetDecoder<object>(dataType).DecodeAsType(_dataReader, ordinal, jsonValueType);
			        return (TResult)jsonValue;
		        }
